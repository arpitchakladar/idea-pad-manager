...
void Page::createPage(
			...
			[&](auto &&RowData) -> auto {
				...
				if constexpr (std::is_same_v<T, RowCustom>) {
					...
					InfoTableValues.push_back(ftxui::Renderer(CustomComponent,
						[RowData = std::forward<decltype(RowData)>(
							 RowData)]() -> ftxui::Element {
							return get<1>(RowData)->Render();
						}));
	...
	m_PageComponent = ftxui::Renderer(InfoTableValuesComponent,
		[&, InfoTableValuesComponent, InfoTableLabels, UpdateCanvas, DrawCanvas,
			Delta, Title = std::move(Title)]() -> ftxui::Element {
			...
			return ftxui::vbox({
					   ...
					   ftxui::hbox({
						   ...
						   ftxui::size(
							   ftxui::WIDTH, ftxui::EQUAL, CanvasWidth) |
						   ftxui::center,
					   ftxui::filler(),
				   }) |
				ftxui::yflex;
